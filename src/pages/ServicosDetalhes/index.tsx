...
import InfoCardContainer from '../../componentes/InfoCards';
  ...
  return (
    <div  ref={sectionRef} style={{ display: 'flex', flexDirection: 'column' }}>
      <animated.section style={{ ...fadeIn, position: 'relative', height: '300px', display: 'flex', textAlign: 'start', color: 'white' }}>
        ...
        <p style={{fontSize: 20, color: 'gray'}}>ADVOGADOS ESPECIALIZADOS</p>
      <h1 style={{fontSize: 50}}>{serviceData.title}<span style={{color: "#8B173B"}} >.</span></h1>
        </div>
      </animated.section>
      ...
      <animated.section style={{ ...fadeIn, backgroundColor: '#FFFFFF', color: '#0E395E', padding: '50px', height: '600px', display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <img src={serviceData.image} height={390} width={470} style={{ marginLeft: 20, marginTop: 10, borderRadius: 20 }} />
      </animated.section>
      <animated.section className="saiba-mais-section" style={fadeIn}>
        <div className="container">
          <h1 style={{ fontSize: 40,margin: 10, color: '#0E395E', alignSelf: 'start' }}>
          O que fazemos<span style={{ color: "#8B173B" }}>?</span>
          </h1>
          
          <InfoCardContainer cardData={serviceData.cardData} />
        </div>
        <Button
          style={{
      ...
      <animated.section className="saiba-mais-section" style={{ ...fadeIn, display: 'flex', overflowX: 'auto', whiteSpace: 'nowrap' }}>
  {areas.map(area => (
    <Card 
      key={area.title}
      title={area.title}
      ...
      style={{ display: 'inline-block', marginRight: '16px' }} // Estilo para cada card
    />
  ))}
</animated.section>
